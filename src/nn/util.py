...
from neuralnetwork import forward_prop
import os


def predict_probs(X, params, activation):
    """
    Helper function to get class 1 probabilities from the network. 
    
    EXPLANATION OF WHY WE DO THIS: h_theta_bar in our nn
    calculates the raw, unnormalized scores. then it passes through our softmax function, 
    which squishes those numbers so that they sum to 1.0. bc we set num_classes=2, the 
    resulting h_theta variable is a 2D matrix of shape (number_of_patients, 2) with...
        Column 0: Probability the patient is Non-Diabetic
        Column 1: Probability the patient is Diabetic
    bc of this, slicing h_theta[:, 1] gives us 1D array of the positive-class probs
    for every single patient in the test set.
    """
    softmax_labels = np.zeros((X.shape[0], 2))
    
    # run forward pass
    _, h_theta, _ = forward_prop(
        X, softmax_labels, params, 
