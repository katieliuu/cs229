...
def backward_prop(data, one_hot_labels, params, forward_prop_func, activation='sigmoid', dropout_rate=0.0, reg=0.0, sample_weights=None): # <-- added sample_weights
    ...
    # Apply dropout mask to gradients (don't update dead neurons)
    ...
    # Apply L2 Weight Decay (Regularization) directly here
...
def one_hot_labels(labels, num_classes=2): # <-- CHANGED: Added num_classes=2 instead of hardcoded 10
...
def run_train_test(name, all_data, all_labels, backward_prop_func, num_epochs, plot=True):
    params, cost_train, cost_dev, accuracy_train, accuracy_dev = nn_train(
        ...
        num_hidden=300, learning_rate=5, num_epochs=num_epochs, batch_size=1000, num_classes=2 # Pass num_classes down
    )

    # save W1, W2, b1, b2 into a .npz file (e.g., baseline.npz)
    np.savez(f'{name}_params.npz', **params)
    print(f"Saved parameters to {name}_params.npz")
...
def main(plot=True):
    # <-- CHANGED: Removed argparse completely. Set variables directly.
    num_epochs = 30

    np.random.seed(100)
    train_data, train_labels = read_data('./images_train.csv', './labels_train.csv')
    # convert labels to one-hot embeddings e_y.
    train_labels = one_hot_labels(train_labels, num_classes=2)
    
    p = np.random.permutation(train_data.shape[0]) # Avoid hardcoded 60000 limit
    train_data = train_data[p,:]
    train_labels = train_labels[p,:]

    # Simplified train/dev split for flexibility 
    split_idx = int(train_data.shape[0] * 0.2) # use 20% for dev
    dev_data = train_data[0:split_idx,:]
    dev_labels = train_labels[0:split_idx,:]
    ...
    # convert labels to one-hot embeddings e_y.
    test_labels = one_hot_labels(test_labels, num_classes=2)
